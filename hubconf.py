...
def _load_model(
    ...
    ctx = context.InferenceContext(
        settings.training.vocab, token_type=settings.training.token_type, use_blank=not use_frame
    )
    ...
    model.streaming()
    if use_frame:
        engine = inference.FrameInferenceEngine(
        ...
        engine = inference.SequenceInferenceEngine(
            settings.audio.sample_rate,
            model,
            zmuv_transform,
            negative_label=ctx.negative_label,
