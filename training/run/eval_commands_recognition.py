...
def main():
    """
    This script uses every GPU on the machine to train every models in this repo
    for detecting gsc commands and compare their performance
    ...
    apb.add_options(
        ArgOption("--num_iterations", type=int, default=1, help="number of experiments to run"),
        ArgOption("--dataset_path", type=str, default="/data/kws/gsc/v1"),
    )
    ...
    workbook = Workbook()
    now = datetime.now()
    dev_sheet = workbook.create_sheet("dev", 0)
    test_sheet = workbook.create_sheet("test", 1)

    model_types = ["res8", "las", "lstm", "mobilenet"]
    col_index_map = {"res8": "B", "las": "C", "lstm": "D", "mobilenet": "E"}
    dev_sheet["A2"] = "mean"
    dev_sheet["A3"] = "std"
    dev_sheet["A4"] = "p90"
    dev_sheet["A5"] = "p95"
    ...
    for i in range(args.num_iterations):
        ...
        for model_type in model_types:
            ...
            # exp_execution = os.system(
            #     "python -m training.run.pretrain_gsc --model "
            #     + model_type
            #     + " --workspace "
            #     + workspace_path
            #     + " 2>&1 | tee "
