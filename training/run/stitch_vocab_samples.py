...
from howl.data.dataset_loader.dataset_loader import WakeWordDatasetLoader
...
def main():
    ...
    parser.add_argument(
        "--disable-detect-keyword",
        action="store_false",
        help="disable keyword detection based verifcation",
    )

    args = parser.parse_args()
    aligned_ds_path = Path(args.aligned_dataset)
    stitched_ds_path = (
        aligned_ds_path / "stitched"
        if args.stitched_dataset == ""
        else Path(args.stitched_dataset)
    )
    ...
    train_ds, dev_ds, test_ds = WakeWordDatasetLoader().load_splits(
        aligned_ds_path, **ds_kwargs
    )

    # stitch vocab samples
    stitcher = WordStitcher(vocab=vocab, detect_keyword=args.disable_detect_keyword)
    stitcher.stitch(
        args.num_stitched_samples, stitched_ds_path, train_ds, dev_ds, test_ds
    )

    # split the stitched samples
    stitched_train_ds, stitched_dev_ds, stitched_test_ds = stitcher.load_splits(
        *args.stitched_dataset_pct
    )
