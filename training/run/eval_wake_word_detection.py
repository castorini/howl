...
def main():
    ...
    sample command:
    python -m training.run.eval_wake_word_detection --num_models 10 --hop_size 0.05 \
    --exp_type hey_firefox --dataset_path "x" --noiseset_path "y"
    ...
    apb.add_options(
        ...
        ArgOption("--exp_type", type=str, choices=["hey_firefox", "hey_snips"], default="hey_firefox"),
        ArgOption("--seed", type=int, default=0),
        ArgOption("--noiseset_path", type=str, default="/data/MS-SNSD"),
    )
    ...
    for _ in range(args.num_models):
        ...
        command = (
            "python -m training.run.train --model res8 --workspace " + workspace_path + "  -i " + args.dataset_path
        )
    ...
    for seed in seeds:
        for _, threshold in enumerate(thresholds):
            ...
            workspace_path = get_workspace_path(args.exp_type, seed)
            command = (
                "python -m training.run.train --eval --model res8 --workspace "
                + workspace_path
                + "  -i "
                + args.dataset_path
            )
